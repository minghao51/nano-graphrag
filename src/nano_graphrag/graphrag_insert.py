    ...
    async def _ainsert_documents(
        ...
        try:
            ...
            if force_rebuild:
                ...
                changed_doc_ids = [
                    doc_id
                    for doc_id, existing_doc in zip(
                        normalized_docs.keys(), existing_docs, strict=False
                    )
            ...
            try:
                ...
                if has_graph_data:
                    if self.enable_community_reports:
                        ...
                        logger.info(
                            "community_report_skipped", reason="enable_community_reports=False"
                        )
                        await self.chunk_entity_relation_graph.clustering(
                            self.graph_cluster_algorithm,
                            affected_node_ids=affected_entity_ids,
                ...
                if manifest_entity_count > 0 and self.chunk_entity_relation_graph is not None:
                    ...
                    results = await asyncio.gather(
                        *[
                            self.chunk_entity_relation_graph.has_node(eid)
