...
class ClusteringBackend(Protocol):
    async def cluster(self, storage, affected_node_ids: set[str] | None = None) -> None: ...
...
class LeidenClusteringBackend:
    def _cluster_data_to_subgraphs(self, storage, cluster_data: dict[str, list[dict[str, Any]]]):
    ...
    async def cluster(self, storage, affected_node_ids: set[str] | None = None) -> None:
        ...
        node_communities: dict[str, list[dict[str, Any]]] = defaultdict(list)  # type: ignore[assignment]
        levels = defaultdict(set)
