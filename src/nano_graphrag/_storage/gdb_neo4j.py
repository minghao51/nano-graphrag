...
class Neo4jStorage(BaseGraphStorage):
    ...
    async def node_degrees_batch(self, node_ids: list[str]) -> list[int]:
        ...
        result_dict = dict.fromkeys(node_ids, 0)
        async with self.async_driver.session() as session:
            result = await session.run(
                f"""
    ...
    async def get_nodes_batch(self, node_ids: list[str]) -> list[dict | None]:
        ...
        result_dict = dict.fromkeys(node_ids)
                ...
                async for record in result:
                    ...
                    if len(edges_data) == 1:
                        ...
                        combined["description"] = GRAPH_FIELD_SEP.join(
                            sorted({d for d in all_descriptions if d})
                        )
                        combined["weight"] = total_weight
                        combined["source_id"] = GRAPH_FIELD_SEP.join(
                            sorted({s for s in all_source_ids if s})
                        )
    ...
    async def get_nodes_edges_batch(self, node_ids: list[str]) -> list[list[tuple[str, str]]]:
        ...
        result_dict: dict[str, list[tuple[str, str]]] = {node_id: [] for node_id in node_ids}
