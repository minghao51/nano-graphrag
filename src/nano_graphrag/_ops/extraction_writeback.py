...
"""
    ...
    try:
        ...
        for idx, (entity_name, _entity_type, _) in enumerate(entity_texts, 1):
...
async def _write_extraction_manifest(
    ...
    tokenizer_wrapper,
    global_config: dict,
    chunks: dict[str, TextChunkSchema] | None = None,
    ...
    if aliases_to_extract:
        ...
        for i in range(0, len(aliases_to_extract), batch_size):
            batch_entities = []
            for _entity_id, entity in aliases_to_extract[i : i + batch_size]:
