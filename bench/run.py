...
from bench import BenchmarkConfig, ExperimentRunner  # noqa: E402
...
Examples:
  ...
  python -m bench --config experiments/configs/core/benchmark_hotpotqa.yaml --name my_experiment

  # Limit to 10 samples
  python -m bench --config experiments/configs/core/benchmark_hotpotqa.yaml --max-samples 10

  # Only run local mode
  python -m bench --config experiments/configs/core/benchmark_hotpotqa.yaml --modes local
        """,
    )

    parser.add_argument("--config", "-c", required=True, help="Path to benchmark config YAML file")
    parser.add_argument("--name", "-n", help="Override experiment name")
    ...
    if dotenv_path.exists():
        ...
        if llm_model and not llm_model.startswith("encrypted:"):
            ...
            print(
                ...
                "Run with: dotenvx run -- uv run python -m bench ..."
            )
            sys.exit(1)
    else:
